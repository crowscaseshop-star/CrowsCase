    ...
    el.innerHTML =
        ...
        S.users.map(function (u) {
          ...
          return '<tr>' +
            '<td><div class="flex"><span class="avatar">' + esc(u.name.charAt(0)) + '</span><div>' +
              ...
              '<div style="font-size:11px;color:var(--muted-2)">' + esc(u.phone || '—') + '</div></div></div></td>' +
      ...
      '<div class="grid g-2">' +
        ...
        '<div class="card"><div class="card-head"><h3>◆ แนะนำเพิ่มเติมด้านความปลอดภัย</h3></div><div class="stack">' +
          ...
          '</div>' + UI.tip('ระบบนี้เก็บข้อมูลไว้ในเครื่องที่เปิดใช้งาน (localStorage) — เหมาะกับการใช้ในร้าน 1 เครื่อง หากต้องการให้พนักงานหลายคนเห็นข้อมูลเดียวกันแบบเรียลไทม์ ต้องต่อฐานข้อมูลบนเซิร์ฟเวอร์เพิ่ม') +
        '</div>' +
      '</div>';
    ...
    $$('[data-udel]').forEach(function (b) {
      b.onclick = function () {
        ...
        UI.confirmBox('ลบบัญชีพนักงาน', 'ลบบัญชี <b>' + esc(u.name) + '</b> ออกจากระบบ?<br>' +
          '<span style="font-size:12px;color:var(--muted-2)">แนะนำให้ใช้การ “ระงับบัญชี” แทน เพื่อคงประวัติการขายให้ตรวจสอบได้</span>',
          'ลบบัญชี', function () {
